...
import { Clock, Plus, Save, Calendar, Info } from 'lucide-react';
import { DAYS, PERIODS, SUBJECTS } from '../constants';
...
const SchedulePage: React.FC<{ role: string, username: string }> = ({ username }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
        ...
        <div className="flex gap-2">
           <button 
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${
              isEditing ? 'bg-emerald-600 text-white shadow-lg' : 'bg-blue-600 text-white shadow-lg shadow-blue-200'
            }`}
          >
                ...
                <tr key={period} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-6 bg-slate-50 border-r border-slate-100">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center font-black text-sm">{period}</div>
      ...
      {!hasData && !isEditing && (
        <div className="p-8 bg-blue-50 rounded-[2rem] border border-dashed border-blue-200 flex items-center gap-4 text-blue-600">
           <Info size={24} />
